...
def build_single_player_figure(player_data, player_name, team_color, highs):
    ...
    fig.update_layout(
        polar=dict(
            ...
            angularaxis=dict(
                tickfont=dict(size=14, family="Playfair Display, Georgia, serif"),
                gridcolor="rgba(128,128,128,0.15)",
            ),
...
def build_comparison_figure(
    player1_data, player1_name, player1_color,
    player2_data, player2_name, player2_color,
    combined_highs,
):
    fig = go.Figure()
    fig.add_trace(build_radar_trace(player1_data, player1_name, player1_color, combined_highs))
    fig.add_trace(build_radar_trace(player2_data, player2_name, player2_color, combined_highs))
    fig.update_layout(
        polar=dict(
            radialaxis=dict(
                visible=True,
